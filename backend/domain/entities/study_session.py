"""
StudySession 도메인 엔티티
학습 모드 세션을 표현하는 도메인 로직
"""
...
class StudySession:
    ...
    테스트 모드와 달리 Test와 Result를 생성하지 않고 학습 기록만 저장
    """
    ...
    def __repr__(self) -> str:
        """문자열 표현"""
        level_str = self.level.value if self.level else "None"
        types_str = ", ".join([qt.value for qt in self.question_types]) if self.question_types else "None"
        return (
            f"StudySession(id={self.id}, user_id={self.user_id}, "
            f"study_date={self.study_date}, study_hour={self.study_hour}, "
            f"level={level_str}, question_types=[{types_str}], "
            f"accuracy={self.get_accuracy_percentage():.1f}%)"
        )
