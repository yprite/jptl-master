...
class TestAdminStatisticsAPI:
    """Admin Statistics API 엔드포인트 테스트"""
    ...
    def test_get_admin_statistics_success(self, app_client, temp_db, admin_user):
        """어드민 통계 조회 성공 테스트"""
        from backend.infrastructure.config.database import Database
        from backend.infrastructure.repositories.user_repository import SqliteUserRepository
        from backend.infrastructure.repositories.question_repository import SqliteQuestionRepository
        ...
        with patch('backend.presentation.controllers.admin.get_database') as mock_get_db, \
            ...
            # 사용자 생성 (활성 사용자 2명, 비활성 사용자 1명)
            user1 = User(id=None, email="user1@example.com", username="user1", target_level=JLPTLevel.N5)
            user2 = User(id=None, email="user2@example.com", username="user2", target_level=JLPTLevel.N4)
            user3 = User(id=None, email="user3@example.com", username="user3", target_level=JLPTLevel.N5)
            user1 = user_repo.save(user1)
            user2 = user_repo.save(user2)
            user3 = user_repo.save(user3)

            # user1과 user2는 테스트를 받았으므로 활성 사용자
            user1.total_tests_taken = 2
            user2.total_tests_taken = 1
            user1 = user_repo.save(user1)
            user2 = user_repo.save(user2)

            # 문제 생성 (N5 3개, N4 2개)
            question1 = Question(id=0, level=JLPTLevel.N5, question_type=QuestionType.VOCABULARY,
                               question_text="問題1", choices=["1", "2", "3", "4"], correct_answer="1",
                               explanation="説明1", difficulty=1)
            question2 = Question(id=0, level=JLPTLevel.N5, question_type=QuestionType.GRAMMAR,
                               question_text="問題2", choices=["1", "2", "3", "4"], correct_answer="2",
                               explanation="説明2", difficulty=2)
            question3 = Question(id=0, level=JLPTLevel.N5, question_type=QuestionType.READING,
                               question_text="問題3", choices=["1", "2", "3", "4"], correct_answer="3",
                               explanation="説明3", difficulty=1)
            question4 = Question(id=0, level=JLPTLevel.N4, question_type=QuestionType.VOCABULARY,
                               question_text="問題4", choices=["1", "2", "3", "4"], correct_answer="1",
                               explanation="説明4", difficulty=3)
            question5 = Question(id=0, level=JLPTLevel.N4, question_type=QuestionType.GRAMMAR,
            ...
            # 테스트 생성
            test1 = Test(id=0, title="Test 1", level=JLPTLevel.N5,
                        questions=[question1, question2, question3], time_limit_minutes=60)
            test1 = test_repo.save(test1)
            test2 = Test(id=0, title="Test 2", level=JLPTLevel.N4,
                        questions=[question4, question5], time_limit_minutes=60)
            test2 = test_repo.save(test2)

            # 결과 생성 (평균 점수 계산: (70 + 80) / 2 = 75)
            result1 = Result(id=0, test_id=test1.id, user_id=user1.id, score=70.0,
            ...
            result2 = Result(id=0, test_id=test2.id, user_id=user2.id, score=80.0,
            ...
            # 통계 조회
            response = app_client.get("/api/v1/admin/statistics")
            assert response.status_code == 200
            data = response.json()
            assert "success" in data
            assert "data" in data
            ...
            assert stats["users"]["total_users"] == 4  # admin + user1 + user2 + user3
            assert stats["users"]["active_users"] == 2  # user1, user2
            ...
            assert "by_level" in stats["questions"]
            assert stats["questions"]["by_level"]["N5"] == 3
            assert stats["questions"]["by_level"]["N4"] == 2

            # 학습 데이터 통계 검증
            assert "learning_data" in stats
            assert "total_results" in stats["learning_data"]
            assert stats["learning_data"]["total_results"] == 2

    def test_get_admin_statistics_unauthorized(self, app_client, temp_db):
        """어드민 통계 조회 - 인증되지 않은 사용자 테스트"""
        from backend.infrastructure.config.database import Database
    ...
    def test_get_admin_statistics_forbidden(self, app_client, temp_db):
        """어드민 통계 조회 - 일반 사용자 접근 테스트"""
        from backend.infrastructure.config.database import Database
        from backend.infrastructure.repositories.user_repository import SqliteUserRepository
        from backend.domain.entities.user import User
        from backend.domain.value_objects.jlpt import JLPTLevel
        ...
        with patch('backend.presentation.controllers.admin.get_database') as mock_get_db, \
            ...
            # 일반 사용자 생성 및 로그인
            user_repo = SqliteUserRepository(db=db)
            user = User(id=None, email="user@example.com", username="user", target_level=JLPTLevel.N5)
            user_repo.save(user)
