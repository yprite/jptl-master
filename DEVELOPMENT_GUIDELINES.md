...
6. **테스트 실행**: `./run_tests.sh` 스크립트를 사용하여 모든 테스트가 통과하는지 확인합니다.
   ...
   - 프론트엔드: 커버리지 80% 이상 + 주요 플로우 100% 확인 필수
...
- 프론트엔드: 최소 80%의 코드 커버리지 + 주요 사용자 플로우 100% 커버리지
- 새로운 기능 추가 시 반드시 해당 영역의 테스트 코드를 작성합니다.

### 3.3-1 TypeScript 타입 체크 (프론트엔드 필수)
- **타입 체크 필수**: 모든 프론트엔드 코드 변경 시 TypeScript 타입 체크를 반드시 수행합니다
- **자동 실행**: `run_tests.sh` 스크립트 실행 시 프론트엔드 테스트 전에 자동으로 타입 체크가 수행됩니다
- **타입 에러 시 작업 중단**: 타입 체크가 실패하면 모든 작업을 중단하고 타입 에러를 수정한 후 다시 진행합니다
- **수동 실행**: 개발 중 빠른 피드백을 위해 `npm run typecheck` 명령어로 수동 실행 가능합니다
- **test:ci 통합**: `npm run test:ci` 실행 시 자동으로 타입 체크를 포함하여 실행됩니다
- **타입 체크 목적**: 
  - 런타임 에러 방지: 컴파일 타임에 타입 에러를 발견하여 런타임 에러를 예방합니다
  - 코드 품질 향상: 타입 안정성을 보장하여 버그를 줄이고 코드 가독성을 향상시킵니다
  - 개발 생산성 향상: IDE 자동완성 및 타입 힌트를 통해 개발 속도를 높입니다

**중요**: TypeScript 타입 체크는 테스트 실행 전 필수 단계이며, 타입 에러가 있으면 테스트를 실행하지 않습니다.
