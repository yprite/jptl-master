...
describe('FlashcardUI', () => {
  ...
  it('should display first vocabulary on initial load', () => {
    ...
    const flashcardUI = screen.getByTestId('flashcard-ui');
    const wordElement = flashcardUI.querySelector('.flashcard-word');
    expect(wordElement).toBeInTheDocument();
    expect(wordElement?.textContent).toBe('こんにちは');
  });

  it('should flip card when clicking on it', () => {
    render(<FlashcardUI vocabularies={mockVocabularies} />);

    const card = screen.getByTestId('flashcard-ui').querySelector('.flashcard');
  ...
  it('should navigate to next vocabulary', () => {
    ...
    const wordElement = flashcardUI.querySelector('.flashcard-word');
    expect(wordElement).toBeInTheDocument();
    expect(wordElement?.textContent).toBe('ありがとう');
  });
  ...
  it('should reset flip state when navigating to next card', async () => {
    render(<FlashcardUI vocabularies={mockVocabularies} />);
    ...
    // 카드가 뒤집히지 않은 상태로 표시되어야 함 (front가 보여야 함)
    // useEffect가 실행되어 카드가 리셋되므로 약간의 대기 시간 필요
    await waitFor(() => {
      const flashcardUI = screen.getByTestId('flashcard-ui');
      const cardElement = flashcardUI.querySelector('.flashcard');
      // 카드가 뒤집히지 않았으므로 'flipped' 클래스가 없어야 함
