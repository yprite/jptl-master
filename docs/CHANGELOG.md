...
### Added
JLPT_N5_UI_리디자인: JLPT N5 6주 완성 프로그램 UI 리디자인 완료 (2025-01-12)
- P0 작업: 모던 사이드바 및 헤더 레이아웃, 대시보드 UI (KPI 카드, 액션 그리드, CTA 배너), 오늘의 미션 페이지, 오답 노트 페이지, SRS 복습 리스트, 디자인 시스템 구현
- P1 작업: 테스트 모드 종료 화면 개선 (해설, 오답 저장), 학습 모드 플로우 (개념 카드 → 예제 → 미니 퀴즈), 성능 분석 페이지 (약점 TOP3, 파트별 정확도, 주간 변화, 추천 학습)
- P2 작업: 공유 가능한 리포트 카드 이미지 생성 (Web Share API), 배지 시스템 (주차 완료, 연속 학습 3/7/14일, 6주 완료 배지, 토스트/모달 알림), 온보딩 플로우 (3단계 질문 후 Today Mission으로 이동)
- 디자인 시스템: Button, Card, Badge, Progress, Tabs, Modal, Toast, Skeleton, Input 컴포넌트 구현
- 반응형 디자인: 데스크톱/태블릿/모바일 지원
- 모든 컴포넌트 TypeScript 타입 안정성 확보

깊은_E2E_시나리오_검증_구현: 깊은 E2E 시나리오 검증 테스트 추가 (2025-01-12)
- 학습 계획 진도율 깊은 검증: Day 완료 시 진도율 계산 및 UI 반영 확인
- 일일 목표 달성률 깊은 검증: 학습 세션 완료 시 달성률 계산 및 백엔드 검증
- 학습 이력 저장 및 조회 깊은 검증: 세션 완료 후 이력 저장 및 조회 확인
- 체크리스트 상태 동기화 깊은 검증: 학습 모드 완료 후 체크리스트 업데이트 확인
- 복합 플로우 데이터 일관성 검증: 여러 기능 연속 사용 시 데이터 일관성 확인
- 백엔드 데이터 검증 통합: E2E 테스트에서 백엔드 API 직접 호출하여 데이터 검증
- 계산 로직 엣지 케이스 검증: 진도율/달성률 계산의 엣지 케이스 검증
- 여러 복합 시나리오 추가: 테스트→오답노트→반복학습, 학습계획→체크리스트→학습, 일일목표→여러세션→달성, 성능분석→학습계획 연동, 어휘학습→복습→이력, 여러주차 완료, 프로필 수정 등
- StudyUI 컴포넌트 제출 버튼에 data-testid 추가
...
- DailyChecklistUI 컴포넌트 구현 (Day 1~42 일일 체크리스트)
- 로그인 후 자동으로 학습 계획 대시보드 표시
- 학습 진행 상태 추적 기능 (로컬 스토리지 기반)
- 주차별 학습 목표, 방법, 포인트 표시
- 일일 체크리스트 (단어, 문법, 독해, 청해) 기능
- 학습 완료 상태 저장 및 진행률 계산
- 학습 계획 관련 컴포넌트 테스트 작성
- App.tsx에 학습 계획 화면 통합 및 네비게이션 개선
...
- 모든 테스트 timeout을 15초로 증가하여 안정성 향상
- 전역 테스트 timeout을 60초로 설정
- 회원가입 후 networkidle 대기 추가로 페이지 로드 안정화
- 테스트 셀렉터를 실제 UI에 맞게 수정 (N5 진단 테스트 → JLPT 학습 플랫폼, 테스트 시작 → 테스트 모드)
- 모든 E2E 테스트가 통과하도록 수정
...
리스닝_문제_자동_TTS_생성: 리스닝 문제 생성/수정 시 자동으로 TTS 오디오 생성 (2025-01-04)
- TTSService 도메인 서비스 구현 (gTTS 기반)
- 리스닝 문제 생성 시 자동으로 TTS 오디오 생성
- 리스닝 문제 수정 시 텍스트 변경되면 자동으로 TTS 재생성
- 자동 캐싱 기능 (같은 텍스트는 재생성하지 않음)
- Question 엔티티에 audio_url 필드 추가
- 데이터베이스 스키마에 audio_url 컬럼 추가
- 백엔드 정적 파일 서빙 설정 (FastAPI StaticFiles)
- 프론트엔드 TestUI에 리스닝 문제용 오디오 플레이어 추가
- 오디오 파일 업로드 API 엔드포인트 구현
- TTS 서비스 단위 테스트 작성 (9개 테스트)
- 모든 테스트 통과 및 커버리지 유지
